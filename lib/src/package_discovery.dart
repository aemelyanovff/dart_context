...
@Deprecated('Use discoverPackages instead')
typedef WorkspacePackage = LocalPackage;
...
class WorkspaceInfo {
  ...
  LocalPackage? findPackageForPath(String filePath) {
    final result = DiscoveryResult(rootPath: rootPath, packages: packages);
    return result.findPackageForPath(filePath);
  }
...
@Deprecated('No longer used - all folders are treated as package collections')
enum WorkspaceType {
  melos,
  pubWorkspace,
  single,
...
/// @deprecated No longer used in new architecture.
@Deprecated('No longer used - Melos config is not needed for package discovery')
class MelosConfig {
  const MelosConfig({
    required this.name,
...
Future<WorkspaceInfo?> detectWorkspace(String startPath) async {
  final result = await discoverPackages(startPath);
  if (result.packages.isEmpty) {
    return null;
  }
  return WorkspaceInfo(
    rootPath: result.rootPath,
    type: result.packages.length == 1
        ? WorkspaceType.single
        : WorkspaceType.pubWorkspace,
    packages: result.packages,
  );
}

/// @deprecated Use [discoverPackagesSync] instead.
@Deprecated('Use discoverPackagesSync instead')
WorkspaceInfo? detectWorkspaceSync(String startPath) {
  final result = discoverPackagesSync(startPath);
  if (result.packages.isEmpty) {
    return null;
  }
  return WorkspaceInfo(
